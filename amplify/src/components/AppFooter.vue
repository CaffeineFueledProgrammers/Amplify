<template>
    <v-footer height="20vh">
        ...
        <a
            ...
            :href="item.href"
            :title="item.title"
            class="d-flex mx-2 align-end justify-end amplify-social-link"
            rel="noopener noreferrer"
            target="_blank"
        >
            <v-icon :icon="item.icon" :size="item.icon === '$amplify' ? 24 : 16" />
        </a>

        <div class="text-caption text-disabled" style="position: absolute; right: 10%">
            &copy; {{ new Date().getFullYear() }} <span class="d-none d-sm-inline-block">Amplify</span>
            —
            <a
